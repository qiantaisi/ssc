var config = {
    DEBUG: false,
    ToastTimeLength: 3000,
    basePath: ""
};
var Tools = {
    setCookie: function(key, value, option) {
        $.fn.cookie(key, value, option);
    },
    ...
    toastRedirect: function (str, time, url) {
        if (typeof(str) == "string" && typeof(time) == "number" && typeof(url) == "string") {
            // $.toast(str, time, url);
            // $.toast(str, time);
            layer.open({
                content: str,
    ...
    confirm: function(content, btnYes, btnNo, yesCallback, noCallback) {
        //询问框
        layer.open({
            content: content,
            btn: [btnYes, btnNo],
    ...
    confirm2: function(content, btnYes, btnNo, title, yesCallback, noCallback) {
        //询问框
        layer.open({
            skin: "layer-skin-888",
            content: content,
    ...
    alertCallback: function(str, func) {
        //询问框
        layer.open({
            content: str,
            btn: ['确定'],
    ...
    hideLoading: function() {
        // $.hidePreloader();
        if (null != Tools.loadingIndex) {
            layer.close(Tools.loadingIndex);
        }
        // layer.closeAll();
    },
    log: function (str) {
...
var PatternUtils = {
    ...
    checkPhone: function(telephone) {
        ...
        if (telephone.match(/^((13[0-9])|(15[^4])|(18[0,2,3,5-9])|(17[0-8])|(147))\d{8}$/)) {
            return true;
        }
        return false;
    },
    checkEmail: function(email) {
        ...
        if (/^[-_A-Za-z0-9]+@([_A-Za-z0-9]+.)+[A-Za-z0-9]{2,3}$/.test(email)) {
            return true;
        }
        return false;
    },
    checkAccount: function(account) {
        ...
        if (/^[a-zA-Z][0-9a-zA-Z]{5,11}$/.test(account)) {
            return true;
        }
        return false;
    },
...
function getListShu(arr,x){
    ...
    if(x > 0 && list.length > 0){
        for(var i = 0; i < x; i++){
            var strX = list.split(" ")[i];
            if(strX.length > 1){
                sum =parseInt(strX.split(",").length);
            }else{
                sum =1;
            }
            tmp *= sum;
...
if (date.getTime() < (new Date('2017-1-28 00:00:00')).getTime()) {   //返回距 1970 年 1 月 1 日到至今之间的毫秒数
    year = 2016;
}
