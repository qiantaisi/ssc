            ...
            <c:forEach items="${promotionList}" var="item">
                <div class="card" onclick="window.location.href='<%=basePath%>yhhd/${item.id}.html'" >
                    <div class="card-header" style="display:block;color:#000">
                        ...
                        <div style="font-size:0.6rem;color:#8D8D8D">活动时间： <fmt:formatDate value="${item.startTime}" pattern="yy/MM/dd" />~<fmt:formatDate value="${item.endTime}" pattern="yy/MM/dd" /></div>
                    </div>
                    <div valign="bottom" class="card-header color-white no-border no-padding" style="padding:15px!important">
