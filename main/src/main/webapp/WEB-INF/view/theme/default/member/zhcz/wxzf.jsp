...
<div class="eveb_content">
    <div class="eveb_nav_sub">
        <ul class="clearfix">
            ...
            <c:if test="${not empty wxzfInfo.systemPayOnlineList}"><li class="on"><a href="javascript:void(0)" onclick="parent.getPage('zhcz/wxzf')">微信支付</a></li></c:if>
    ...
    <div class="eveb_content">
        <form name="wxzfForm" novalidate="novalidate" onsubmit="return false;">
            <ul class="eveb_form">
                <li>
...
<script>
    $(function () {
        $("#money").keyup(function() {
            var minMoney = parseFloat($("form[name='wxzfForm'] input[name='payOnlineId']:checked").data("minmoney"));
            var maxMoney = parseFloat($("form[name='wxzfForm'] input[name='payOnlineId']:checked").data("maxmoney"));
        ...
        $("form[name='wxzfForm'] input[name='payOnlineId']").change(function() {
            var str = '';

            var val = $("form[name='wxzfForm'] input[name='payOnlineId']:checked").val();
            if (val != '') {
                str = $("form[name='wxzfForm'] input[name='payOnlineId']:checked").data("description");
                $("#zfsm").show();
            } else {
//                $("#zfsm").hide();
            }
        ...
        $("form[name='wxzfForm'] input[name='payOnlineId']").eq(0).attr("checked", 'checked').prop("checked", "checked");
        $("#zfsm .bankInfo .name").html($("form[name='wxzfForm'] input[name='payOnlineId']:checked").data("description"));
        $("#money").keyup(function () {
            var obj = this;
        ...
        $("form[name='wxzfForm']").submit(function() {
            var minMoney = parseFloat($("form[name='wxzfForm'] input[name='payOnlineId']:checked").data("minmoney"));
            var maxMoney = parseFloat($("form[name='wxzfForm'] input[name='payOnlineId']:checked").data("maxmoney"));
            var payurl = $("form[name='wxzfForm'] input[name='payOnlineId']:checked").data("payurl");
            ...
            window.open("http://" + payurl + "/pay/payOnlineRequest.html?refer=<%=basePath%>&uid=${userSession.id}&money=" + money + "&payOnlineId=" + $("form[name='wxzfForm'] input[name='payOnlineId']:checked").val());
        });
