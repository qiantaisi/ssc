        ...
        <div class="activity">
            ...
            <ul>
                <c:forEach items="${promotionList}" var="item">
                    <li><a onclick="showContent(this)" data-name="${item.name}" data-time="<fmt:formatDate value="${item.createTime}" pattern="yyyy-MM-dd" />" data-content="${item.content}" href="javascript:void(0)"><img src="<%=basePath%>image/${item.smallImageId}" alt="${item.name}"></a></li>
                </c:forEach>
            </ul>
        </div>
    </div>
