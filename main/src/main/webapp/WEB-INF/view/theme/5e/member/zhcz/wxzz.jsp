...
<div class="eveb_content">
    <div class="eveb_nav_sub">
        <ul class="clearfix">
            ...
            <c:if test="${not empty wxzzInfo.skInfoList}"><li class="on"><a href="javascript:void(0)" onclick="parent.getPage('zhcz/wxzz')">微信转账</a></li></c:if>
    ...
    <div class="eveb_content">
        <form name="wxzzForm" novalidate="novalidate" onsubmit="return false;">
            <ul class="eveb_form">
                <li>
                    <span>充值金额：</span>
                ...
                <li>
                    <span>微信昵称：</span>
                    <input type="text" size="5" name="userWeixinName" class="suminp wxName" id="name">
                    <label for="name" class="error" id="nameError"></label>
                </li>
                <li>
                    <span>收款账号：</span>
                    <select id="payId" name="weixinId">
                        <c:forEach items="${systemWeiXinResult.skInfoList}" var="item">
...
<script>
    function getIdInfo() {
        ...
        if ( $("#payId").val() > 0) {
            ...
            if (imageId) {
                ...
                str += '</p>';
                str += '<div class="name">请使用微信昵称扫码完成付款，或转账给以下微信昵称号</div>';
            }
    ...
    $(function () {
        ...
        $("form[name='wxzzForm']").submit(function() {
            if (checkInput()) {
                return;
            }

            ajaxRequest({
                url: "<%=basePath%>member/submitWeixinzz.json",
                data: $("form[name='wxzzForm']").serialize(),
                beforeSend: function() {
                    parent.showLoading();
                },
