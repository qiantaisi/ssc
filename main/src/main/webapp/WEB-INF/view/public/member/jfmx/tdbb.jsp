...
<div class="help_r_main">
    <div class="eveb_content">
        ...
        <%--<div class="eveb_amount fl">当前小计：余额<span id="pageMoney"> 0 </span>元，积分<span id="pagePoints"> 0 </span>，总计：余额<span id="totalMoney"> 0 </span>元，积分<span id="totalPoints"> 0 </span></div>--%>
...
<script>
    ...
    function getData() {
        ajaxRequest({
            url: "<%=basePath%>agent/ajaxGetUserReport.json",
            data: {
            ...
            success: function (json) {
                ...
                if (json.agentUserReportList.length == 0) {
                    $("#dataTable tbody").html('<tr><td colspan="5" styl="text-align:center;">暂无数据</td></tr>');
                    return;
                }
                ...
                $.each(json.agentUserReportList, function (index, value) {
                    ...
                    str += '<td>' + value.totalDepositMoney + '</td>';
                    str += '<td>' + value.totalWithdrawMoney + '</td>';
                    str += '<td>' + value.totalGameProfitMoney + '</td>';
                    str += '<td>' + value.totalActivityMoney + '</td>';
                    str += '<td>' + value.totalSscFanshuiMoney + '</td>';
                    str += '<td>' + add(add(value.totalGameProfitMoney, value.totalActivityMoney), value.totalSscFanshuiMoney) + '</td>';
                    str += '<td>' + value.totalYjMoney + '</td>';
                    str += '</tr>';
                });
